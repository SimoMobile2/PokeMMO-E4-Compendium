...
import { typeBackgrounds, generateDualTypeGradient, getPrimaryColor } from "./data/pokemonColors";
...
function App() {
  ...
  const colorizeStrategyText = (text) => {
      ...
      const sortedKeywords = Object.keys(colorKeywords).sort((a, b) => b.length - a.length);

      // Costruisci un'espressione regolare che cerca tutte le parole chiave
      // Usa \b per cercare confini di parola e (|) per alternare
      const keywordPattern = new RegExp(`\\b(${sortedKeywords.map(kw => kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`, 'gi');
      ...
      textWithPlaceholders.replace(keywordPattern, (match, keyword, offset) => {
          ...
          const cssClass = colorKeywords[keyword.toLowerCase()] || colorKeywords[keyword]; // Controlla sia case sensitive che lowercase
          parts.push(
  ...
  const handlePokemonCardClick = (pokemon) => {
    ...
    const pokemonStrategy = eliteFourMembers
        .find(member => member.name === selectedMember?.name) // Trova il membro selezionato
  ...
  const selectedPokemonData = pokemonData.find(p => p.name === selectedPokemon?.name);
  const selectedPokemonTypes = selectedPokemonData?.types || [];

  let detailsTitleBackground = typeBackgrounds[selectedPokemon?.name] || typeBackgrounds[""];

  if (!typeBackgrounds[selectedPokemon?.name] && selectedPokemonTypes.length >= 2) {
      detailsTitleBackground = generateDualTypeGradient(selectedPokemonTypes[0], selectedPokemonTypes[1]);
  } else if (!typeBackgrounds[selectedPokemon?.name] && selectedPokemonTypes.length === 1) {
       detailsTitleBackground = typeBackgrounds[selectedPokemonTypes[0]] || typeBackgrounds[""];
  }
  ...
  const renderCurrentStrategyView = (strategy) => {
    if (!strategy || strategy.length === 0) {
        ...
        if (strategyHistory.length === 0) {
             ...
             return <p>Nessuna strategia dettagliata disponibile per questo Pokémon.</p>;
         } else {
    ...
    return (
      <div className="strategy-content-list">
        ...
        {strategy.map((item, index) => {
          ...
          const formattedText = item.player ? colorizeStrategyText(item.player) : null;
          ...
          if (item.type === "step") {
            ...
            const stepClassName = `strategy-step ${hasVariations ? 'has-variations' : ''}`;

            return (
              <div key={index}>
                  <div
                      className={stepClassName}
                      onClick={hasVariations ? () => handleStepClick(item) : undefined} // Chiama handleStepClick solo se ha variazioni
        ...
        {selectedRegion && filteredEliteFour.length > 0 && (
           <div className="cards-container elitefour-container">
             {filteredEliteFour.map((member, i) => {
               const memberBackground = typeBackgrounds[member.type] || typeBackgrounds[""];
            ...
            {pokemonNamesForSelectedTeam.length > 0 ? (
              pokemonNamesForSelectedTeam.map((pokemonName, index) => {
                ...
                if (!pokemon) {
                   return (
                     <PokemonCard
                       ...
                       onClick={() => handlePokemonCardClick({ name: pokemonName, types: [] })}
                ...
                if (!typeBackgrounds[pokemon.name] && pokemonTypes.length >= 2) {
                    nameBackground = generateDualTypeGradient(pokemonTypes[0], pokemonTypes[1]);
                } else if (!typeBackgrounds[pokemon.name] && pokemonTypes.length === 1) {
                     nameBackground = typeBackgrounds[pokemonTypes[0]] || typeBackgrounds[""];
                }
              ...
              <p>Nessun Pokémon definito per il {selectedTeam} di {selectedMember.name}.</p>
            )}
          </div>
        )}
